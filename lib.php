 ...
 * @return string
 */
function theme_skripsian_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_skripsian', 'preset', 0, '/', $filename))) {
    ...
    // skripsian scss.
    $skripsianvariables = file_get_contents($CFG->dirroot . '/theme/skripsian/scss/skripsian/_variables.scss');
    $skripsian = file_get_contents($CFG->dirroot . '/theme/skripsian/scss/default.scss');

    // Combine them together.
    $allscss = $skripsianvariables . "\n" . $scss . "\n" . $skripsian;
 ...
 * @return string
 */
function theme_skripsian_get_extra_scss($theme) {
    $content = '';
 ...
 * @return string
 */
function theme_skripsian_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_skripsian_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/skripsian/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_skripsian_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('skripsian');

    if ($context->contextlevel == CONTEXT_SYSTEM &&
        ($filearea === 'logo' || $filearea === 'loginbgimg' || $filearea == 'favicon' || $filearea == 'footerlogo')) {
        $theme = theme_config::load('skripsian');
